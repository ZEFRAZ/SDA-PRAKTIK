    ...
    do {
        ...
        switch (opsi) {
            ...
            case 2: { // Tambah Peminjam ke Antrian Buku
                ...
                borrowerAddress existingBorrower = bukuArray[index].firstQueue;
                ...
                masukAntrian(&bukuArray[index], newBorrower);
            ...
            case 3: { // Tampilkan Daftar Buku dan Antrian
                ...
                for(i = 0; i < jumlahBuku; i++) {
                    ...
                    printQueue(bukuArray[i].firstQueue);
                }
                pressEnterToContinue();
                clearScreen();
                break;
            ...
            case 4: { // Proses Peminjaman Buku
                ...
                if (!hasQueuedBorrowers) {
                    printf("Tidak ada antrian peminjam pada semua buku.\n");
                    pressEnterToContinue();
                    clearScreen();
                    break;
                }
                ...
                if (bukuArray[index].firstQueue == NULL) {
                ...
                int prioritasAsli = bukuArray[index].firstQueue->prioritas;  // << simpan dulu
                hapusPeminjam(&bukuArray[index], namaTerhapus);
                ...
                if (strcmp(namaTerhapus, "-") != 0) {
                    ...
                    if (bukuArray[index].peminjamAktif == NULL) {
                        ...
                        borrowerAddress last = bukuArray[index].peminjamAktif;
            ...
            case 6: { // Batalkan Antrian
                ...
                printf("\nDaftar Buku:\n");
                for(i = 0; i < jumlahBuku; i++) {
                    printf("%d. %s\n", i+1, bukuArray[i].judul);
                }

                // Pilih buku
                int pilihan = input_int("Pilih nomor buku: ", 1, jumlahBuku);
                int index = pilihan - 1;
                ...
                int pilihanPeminjam = input_int("Pilih nomor peminjam yang akan dibatalkan: ", 1, indexPeminjam - 1);

                // Cari dan hapus peminjam yang dipilih
                char namaTerhapus[50];
                current = bukuArray[index].firstQueue;
                int j;
                for (j = 1; j < pilihanPeminjam; j++) {
                    current = current->next;
                }
                strcpy(namaTerhapus, current->nama);
                
                hapusPeminjamTertentu(&bukuArray[index], namaTerhapus, &bukuArray[index].firstQueue);
            ...
            case 10: {
                ...
                if (jumlahBuku > 0) {
                    ...
                    for (i = 0; i < jumlahBuku; i++) {
                        // Hapus antrian
                        borrowerAddress current = bukuArray[i].firstQueue;
                        while (current != NULL) {
                            borrowerAddress temp = current;
                            current = current->next;
                            free(temp);
                        }
                        bukuArray[i].firstQueue = NULL;
                        
                        // Hapus peminjam aktif
                        current = bukuArray[i].peminjamAktif;
                        while (current != NULL) {
                            borrowerAddress temp = current;
                            current = current->next;
                            free(temp);
