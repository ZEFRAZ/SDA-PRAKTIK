...
static void tampilkanStatusBuku(buku bukuArray[], int index) {
    ...
    // Tampilkan antrian untuk buku ini
    printf("- Antrian: ");
    borrowerAddress p = firstQueue;
    boolean adaAntrian = false;
    while (p != NULL) {
        if (p->targetBuku == &bukuArray[index]) {
            adaAntrian = true;
            printf("-> \"%s\" ", p->nama);
        }
        p = p->next;
...
void runTestScenario(buku bukuArray[], int *jumlahBuku, aktivitasStack *aktivitasTop) {
    ...
    masukAntrian(anggota1, &bukuArray[0]);
    ...
    masukAntrian(dosen1, &bukuArray[0]);
    ...
    masukAntrian(umum1, &bukuArray[0]);
    ...
    masukAntrian(umum1_buku2, &bukuArray[1]);
    ...
    hapusPeminjam(namaTerhapus, &bukuArray[0]);
    ...
    if (strcmp(namaTerhapus, "-") != 0) {
        ...
        infoBorrower(&peminjamAktif, namaTerhapus, PEMINJAM_AKTIF);
        peminjamAktif->next = bukuArray[0].peminjamAktif;
    ...
    hapusPeminjamTertentu("Umum1", &firstQueue);
